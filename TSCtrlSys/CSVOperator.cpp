...
bool CCSVOperator::LoadCSV(const char* path)
{
    ...
    if (pfile)
    {
        ...
        for (; iter != StringMap.end(); ++iter)
        {
            ...
            for (UINT i = 0; i < StringVec.size(); ++i)
            {
...
bool CCSVOperator::GetDouble(u32 uiLine, u32 uiRow, double& dValue)
{
	float tmpVal=0.0;
	if (GetFloat(uiLine, uiRow, tmpVal))
	{
		dValue = (double)tmpVal;
		return true;
	}
	else
	{
...
bool CCSVOperator::SetNumber(u32 uiLine, u32 uiRow, double dValue)
{
	std::string* pKey = GetString(uiLine, uiRow);
	if (pKey)
	{
		char buffer[100];
		memset(buffer, 0, sizeof(buffer));
		sprintf(buffer, "%.3f", (float)dValue);
		pKey->clear();
		*pKey = buffer;
		return true;
	}
