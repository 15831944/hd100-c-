#include "StdAfx.h"
#include "AVTCameraCtrl.h"

#pragma warning(disable: 4100)
CAVTCameraCtrl *g_Carm = NULL;
IMPLEMENT_DYNAMIC(CAVTCameraCtrl, CWnd)
CAVTCameraCtrl::CAVTCameraCtrl()
{
	FGINIT Arg;
...
BEGIN_MESSAGE_MAP(CAVTCameraCtrl, CWnd)
END_MESSAGE_MAP()

// 打开相机 
bool CAVTCameraCtrl::OpenCarm(void)
{
	UINT32 Result = FCE_NOERROR;
...
void CAVTCameraCtrl::Carmlive(void)
{
	m_avtCamera.StartDevice();
}

void CAVTCameraCtrl::ShowBitmap(FGFRAME frame)
{
	// 如果没有分配图像缓冲区
	if(!m_pImageBuffer)
		return;
...
void CAVTCameraCtrl::InitBitmap(UINT32 ImageWidth, UINT32 ImageHeight, BOOL IsColorCamera)
{
...
void WINAPI CAVTCameraCtrl::SnapCallback(void* Context,UINT32 wParam,void* lParam)
{
	FGFRAME frame;
	UINT32 Result = 0;
		...
		case WPARAM_FRAMESREADY:
			do
			{
				...
				if(frame.pData)
				{
					...
					//g_pView->m_ImgStatic.m_pImageBuffer=g_Carm->m_pImageBuffer;
...
void CAVTCameraCtrl::CloseCarm(void)
{
	m_bLivingSnap = FALSE;
...
void CAVTCameraCtrl::SetCarmParam(int P_AVTShutter,int P_AVTBrightness,int P_AVTAVTGain)
{
...
void CAVTCameraCtrl::StopCarm(void)
{
	m_avtCamera.StopDevice();
}
