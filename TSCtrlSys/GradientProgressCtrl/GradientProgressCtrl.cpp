...
CGradientProgressCtrl::CGradientProgressCtrl()
{
	...
	m_clrBkGround=COLORREF(RGB(0xE6,0xE6,0xFA));
...
//	
//	//return TRUE;//
//	CProgressCtrl::OnEraseBkgnd(pDC);
//}
...
void CGradientProgressCtrl::DrawEndUp(CPaintDC *pDC, const RECT &rectClient, const int &nMaxWidth)
{
	RECT	rectFill;		//显示区域
	float	fStep;			//每一步的幅宽
	CBrush	brush;			//显示的颜色画刷
	...
	for(int iOnBand=0;iOnBand<nSteps;iOnBand++)
	{
		...
		VERIFY(brush.DeleteObject());
		//在结束绘制之前，使用背景色填充乘下的的客户区域
		if(rectClient.bottom - rectFill.bottom > nMaxWidth)
		{
...
void CGradientProgressCtrl::OnPaint() 
{
	...
	//超出range
	if(m_nCurrentPosition<m_nLower)
	{
		m_nCurrentPosition=m_nLower;
	}
	if (m_nCurrentPosition>m_nUpper)
	{
	...
	float maxWidth((float)m_nCurrentPosition/(float)m_nUpper*(float)rectClient.bottom);
	//竖直绘制
	DrawEndUp(&dc,rectClient,(int)maxWidth);
	//显示进程条进度文字
	if(m_bShowPercent)
	{
		CString percent;
		percent.Format(_T("%d"),m_nCurrentPosition);
		dc.SetTextColor(m_clrText);
		dc.SetBkMode(TRANSPARENT);
		dc.DrawText(percent,&rectClient,DT_VCENTER|DT_CENTER|DT_SINGLELINE);

		percent.Format(_T("%d"),m_nUpper);
		dc.DrawText(percent,&rectClient,DT_TOP|DT_RIGHT|DT_SINGLELINE);
	}
...
{
	//Get the Position of the Progress
	//nPos=m_nCurrentPosition;
