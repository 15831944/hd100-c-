...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	//m_crTextBKColor       = ::GetSysColor(COLOR_3DFACE);
    m_crGridLineColour    = RGB(192,192,192);
	//m_crFixedBkColour     = RGB(0,0,192);//wsg added 20091014
...
#ifdef _WIN32_WCE
    m_bDoubleBuffer       = TRUE;      //wsg   // Use double buffering to avoid flicker?
#else
	...
	m_bLBtnDownEnable=true;				//add by long 10.01.04
	
    SetupDefaultCells();
    SetGridBkColor(m_cr3DFace);
	//SetFixedBkColor(m_crFixedBkColour);//wsg 20091014 added 
	//SetTextBkColor(m_crTextBKColor);//wsg 20091014 added 
...
void CGridCtrl::SetupDefaultCells()
{
    ...
    m_cellFixedRowDef.SetBackClr(m_cr3DFace);//wsg
    m_cellFixedRowColDef.SetTextClr(m_crWindowText);
    m_cellFixedRowColDef.SetBackClr(m_cr3DFace);//wsg
}
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (nChar == VK_DELETE)
    {
        if (IsCellEditable(m_idCurrentCell.row, m_idCurrentCell.col))
        {
            ...
            SetGridItemText(m_idCurrentCell.row, m_idCurrentCell.col, _T(""));
...
///////////////////wsg added for set now playback index///////////////////////////////////////////////////////
CCellID CGridCtrl::SetSelectCell(int nRow, int nCol)
{
    return SetSelectCell(CCellID(nRow, nCol));
}
///////////////////wsg added///////////////////////////////////////////////////////
CCellID CGridCtrl::SetSelectCell(CCellID cell)
{
    ...
    if (IsValid(idPrev))
    {
        ...
        SetItemState(idPrev.row, idPrev.col,
            GetItemState(idPrev.row, idPrev.col) & ~GVIS_SELECTED);
    ...
    if (IsValid(m_idCurrentCell))
    {
        SetItemState(m_idCurrentCell.row, m_idCurrentCell.col,
            GetItemState(m_idCurrentCell.row, m_idCurrentCell.col) | GVIS_SELECTED);
...
void CGridCtrl::SetSelectRow(CCellID currentCell, 
							 BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
        return;
    ...
    do
    {
        ...
        while (!strLine.IsEmpty())
		{
            ...
            if (IsValid(TargetCell))
            {
                ...
                SetGridItemText(TargetCell.row, TargetCell.col, strCellText);
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    // Initialise column data
    SetGridItemText(0, nColumn, strHeading);
    for (int row = 0; row < m_nRows; row++) 
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    // Set row title
    SetGridItemText(nRow, 0, strHeading);
...
BOOL CGridCtrl::SetGridItemText(int nRow, int nCol, LPCTSTR str)
{
    if (GetVirtualMode())
        return FALSE;
...
void CGridCtrl::ExpandColumnToFit(int Col)
{
    if (GetColumnCount() <= 0)
        return;
            ...
            *end = _T('\0');
            ...
            while (token)
            {
                if (!nSubItem)
                    ...
                    SetGridItemText(nItem, nSubItem, token);
...
int	   CGridCtrl::GetItemDataInt(int nRow, int nCol) const
{
	return atoi(GetItemText(nRow, nCol));
}


double CGridCtrl::GetItemDataDouble(int nRow, int nCol) const
{
	return atof(GetItemText(nRow, nCol));
}
